...
def test_planner_sends_live_xy_and_anchors_lift_height(kind, phase):
    start = np.array([.4, 0., .15])
    live = np.array([.7, .2, .35])
    ...
    node = SimpleNamespace(
        ...
        _effective_object_to_manipulate=lambda _: phase != 'approach',
        ...
        _approach_root_pose=lambda: (np.zeros(3), quat.copy()),
        request_retargeted_keyframe=Mock(return_value=None),
    )
    def select_axis():
        node.box_forward_axis = 'x' if kind == 'bucket' else '-y'
        return True
    node._update_box_forward_axis_from_robot_once.side_effect = select_axis
    ...
    node._update_box_forward_axis_from_robot_once.assert_called_once()
    assert sent['box_forward_axis'] == ('x' if kind == 'bucket' else '-y')
    assert sent['object_to_manipulate'] == (phase != 'approach')
    p = sent['current_box_pose'].pose.position
    np.testing.assert_allclose([p.x, p.y, p.z], [.7, .2, .15] if phase == 'stand_after_pick' else live)
    np.testing.assert_array_equal(live, [.7, .2, .35])
    p = sent['target_box_pose'].pose.position
    np.testing.assert_allclose([p.x, p.y, p.z], [1.4, 0., .15])


@pytest.mark.parametrize('kind', ['box', 'bucket'])
def test_retargeter_refreshes_pickup_xy_but_preserves_lift_height_and_destination(kind):
    ...
    for phase, x in [('stand_before_pick', .4), ('stand_before_pick', .6),
                     ('crouch_to_pick', .7), ('stand_after_pick', .9), ('stand_after_pick', 1.1)]:
        z = .15 if phase != 'stand_after_pick' else .5
        request = SimpleNamespace(
            keyframe_name=f'{phase}_{kind}', object_to_manipulate=True,
            current_box_pose=pose([x, x / 2., z]), target_box_pose=pose([x + 1., 0., .15]),
        ...
        np.testing.assert_allclose(node._current_box_center, [x, x / 2., .15])
