...
class KeyframeRetargetorNode(Node):
    ...
    def _apply_box_ik(self, payload: dict[str, np.ndarray], dst_center: np.ndarray, dst_quat: np.ndarray) -> None:
        ...
        dst_box = BoxFrame(center=np.asarray(dst_center, dtype=np.float64), size=self._box_size_xyz.copy(), quat_wxyz=np.asarray(dst_quat, dtype=np.float64))
        targets = infer_scaled_targets(src_box, dst_box, ee_world)
